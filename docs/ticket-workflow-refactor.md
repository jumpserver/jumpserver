...
后续的类型插件化、多人授权及四类常用审批申请见 [PAM 工单类型插件](ticket-plugins.md)。部署新版还需执行 tickets 0013–0014，并同步部署 Lina。
...
通用 Action 节点、并行、Webhook、代理审批、BPMN、风险引擎等暂未实现。抄送作为可插入流程任意位置的节点提供。

## 模型与执行规则

- `Workflow` 管理名称、业务类型、启用状态及当前发布版本。`WorkflowVersion`、`WorkflowNode`、`WorkflowEdge` 发布后不可修改，只能发布新版本。
...
- 抄送节点配置用户 UUID 列表，到达该节点时只加入仍有效的本组织成员，记录 `cc.added` 事件并在事务提交后通知。抄送权限自该节点起生效，之前的节点或未经过的分支不会提前授予；多个节点累计抄送人，重复选择不会重复通知。
...
| `workflows/options/?type=apply_asset&org_id=...` | 申请人可选的已发布、启用流程 |
| `workflows/{id}/publish/` | 发布，必须提交 `expected_version` 和 `definition` |
| `workflows/{id}/versions/` | 分页查看不可变版本及节点图 |
| `tickets/open/` | 使用 `workflow_id` 和 `request_data` 提交完整资产申请 |
...
流程配置位于 `/tickets/workflows`。设计器支持可视化插入审批、条件和抄送节点、配置真/假分支与汇合、递归组合条件、审批人来源、门槛、超时和操作开关；可查看历史版本并作为新版本发布。后端对完整 DAG 做最终校验。
...
2. 部署后端代码，执行 `python apps/manage.py migrate`，包括 assets 0031、users 0008、tickets 0010–0014、acls 0005。
...
两个命令均支持 `--org-id`；定义命令支持 `--flow-id`，实例命令支持 `--ticket-id`。默认不写入；每个对象单独原子提交，可幂等重跑。四种旧专用工单表在 `tickets.0016_merge_ticket_models` 迁移中搬迁业务字段后删除；历史工单由通用 `Ticket` 和插件读取。
...
`tickets 0014` 将已有流程的全局抄送配置转成起始位置的抄送节点并发布新版本，原发布版本及运行中实例不改动；这些实例的抄送人已在提交时保存到工单。未发布的草稿若曾单独配置全局抄送，原用户 ID 保存在 `migration_notes.unpublished_cc_user_ids`，升级后需在设计器中添加抄送节点再发布。
