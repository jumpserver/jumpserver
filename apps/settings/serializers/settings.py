...
__all__ = ['BaseSettingSerializer', 'BasicSettingSerializer', 'EmailSettingSerializer',
           "EmailContentSettingSerializer", 'LdapSettingSerializer',
           'TerminalSettingSerializer', 'SecuritySettingSerializer']
...
class BaseSettingSerializer(serializers.Serializer):
    ...
    def update(self, instance, validated_data):
        self.update_validated_settings(validated_data)
...
class BasicSettingSerializer(BaseSettingSerializer):
    SITE_URL = serializers.URLField(required=True)
    USER_GUIDE_URL = serializers.URLField(required=False)
    EMAIL_SUBJECT_PREFIX = serializers.CharField(max_length=1024, required=True)


class EmailSettingSerializer(BaseSettingSerializer):
    ...
    EMAIL_FROM = serializers.CharField(max_length=128, required=False)
    EMAIL_RECIPIENT = serializers.CharField(max_length=128, allow_blank='', required=False)
    EMAIL_USE_SSL = serializers.BooleanField(required=False)
    EMAIL_USE_TLS = serializers.BooleanField(required=False)


class EmailContentSettingSerializer(BaseSettingSerializer):
    EMAIL_CUSTOM_USER_CREATED_SUBJECT = serializers.CharField(max_length=1024, required=False, )
    EMAIL_CUSTOM_USER_CREATED_HONORIFIC = serializers.CharField(max_length=1024, required=False, )
    EMAIL_CUSTOM_USER_CREATED_BODY = serializers.CharField(max_length=4096, required=False)
    EMAIL_CUSTOM_USER_CREATED_SIGNATURE = serializers.CharField(max_length=512, required=False)


class LdapSettingSerializer(BaseSettingSerializer):
    ...
    AUTH_LDAP_SEARCH_OU = serializers.CharField(max_length=1024, required=False)
...
class TerminalSettingSerializer(BaseSettingSerializer):
    SORT_BY_CHOICES = (
        ('hostname', _('Hostname')),
        ('ip', _('IP'))
    )
    ...
    TERMINAL_TELNET_REGEX = serializers.CharField(required=False)


class SecuritySettingSerializer(BaseSettingSerializer):
