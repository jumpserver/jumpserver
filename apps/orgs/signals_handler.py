...
def _remove_users(model, users, org):
    with tmp_to_org(org):
        ...
        objects_id_users_id_map = defaultdict(set)

        m2m_field_attr_name = f'{m2m_field_name}_id'
        for relation in relations:
            object_id = getattr(relation, m2m_field_attr_name)
            objects_id_users_id_map[object_id].add(relation.user_id)

        objects = model.objects.filter(id__in=objects_id_users_id_map.keys())
        for obj in objects:
            m2m_changed.send(
                sender=m2m_model, instance=obj, reverse=reverse, model=User,
                pk_set=objects_id_users_id_map[obj.id], using=model.objects.db, action=PRE_REMOVE
            )
        ...
        for obj in objects:
            m2m_changed.send(
                sender=m2m_model, instance=obj, reverse=reverse, model=User,
                pk_set=objects_id_users_id_map[obj.id], using=model.objects.db, action=POST_REMOVE
            )
