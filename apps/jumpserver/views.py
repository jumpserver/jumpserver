...
class IndexView(LoginRequiredMixin, TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not request.user.is_org_admin and not request.user.is_audits:
            return redirect('assets:user-asset-list')
        if not current_org or not current_org.can_admin_by(request.user) and not request.user.is_audits:
