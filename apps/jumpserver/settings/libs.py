...
if REDIS_USE_SSL:
    ...
    REDIS_LAYERS_HOST.update(REDIS_SSL_PARAMS)
...
if REDIS_SENTINEL_SERVICE_NAME and REDIS_SENTINELS:
    ...
    REDIS_LOCATION = '{scheme}://:{password}@{host}:{port}/{db}'.format(
        scheme=scheme, password=CONFIG.REDIS_PASSWORD,
        host=CONFIG.REDIS_HOST, port=CONFIG.REDIS_PORT, db=CONFIG.REDIS_DB_WS
    )
    REDIS_SSL_PARAMS.pop('ssl', None)
    REDIS_LAYERS_HOST['address'] = "%s?%s" % (REDIS_LOCATION, urlencode(REDIS_SSL_PARAMS))
