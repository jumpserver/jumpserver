from common.drf.pagination import JMSLimitOffsetPagination
...
class AssetPaginationBase(JMSLimitOffsetPagination):
    ...
    def get_count(self, queryset):
        ...
        for k, v in self._request.query_params.items():
            if k not in exclude_query_params and v is not None:
                logger.warn(f'Not hit node.assets_amount because find a unknown query_param `{k}` -> {self._request.get_full_path()}')
