...
class KubernetesClient:
    ...
    @staticmethod
    def get_kubernetes_data(app_id, account_id):
...
class KubernetesTree:
    ...
    def as_tree_node(self, app):
        ...
        app_id = str(app.id)
        parent_info = {'asset_id': self.tree_id}
    ...
    def as_account_tree_node(self, account, parent_info):
        ...
        i = self.create_tree_id(pid, 'account_id', account_id)
        parent_info.update({'account_id': account_id})
        node = self.create_tree_node(
            i, pid, name, 'account', parent_info, icon='user-tie'
        )
        return node

    def as_namespace_pod_tree_node(self, name, meta, type, counts=0, is_container=False):
        i = self.create_tree_id(self.tree_id, type, name)
    ...
    def async_tree_node(self, parent_info):
        ...
        tree = []
        data = KubernetesClient.get_kubernetes_data(asset_id, account_id)
        if not data:
            return tree
