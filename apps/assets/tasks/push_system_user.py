...
def push_system_user_to_assets(system_user_id, assets_id, username=None):
    ...
    if username is None and system_user.username_same_with_user:
        # 动态系统用户，把与系统用户关联的所有用户推送到新关联的资产上
        usernames = system_user.users.all().values_list('username', flat=True).distinct()
        ret = []
        for username in usernames:
            ret.append(
                push_system_user_util(system_user, assets, task_name, username=username)
            )
        return ret
