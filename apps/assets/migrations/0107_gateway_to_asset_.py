...
def create_internal_gateway_platform(apps, schema_editor):
    from assets.const import HostTypes
    db_alias = schema_editor.connection.alias
    platform_model = apps.get_model('assets', 'Platform')
    automation_model = apps.get_model('assets', 'PlatformAutomation')
    protocol_model = apps.get_model('assets', 'PlatformProtocol')
    automation = automation_model.objects.using(db_alias).create(
        **HostTypes.get_constrains()[HostTypes.LINUX]['automation']
    )

    platform = platform_model.objects.using(db_alias).create(
        name='Gateway', internal=True, automation=automation
    )

    protocol_model.objects.using(db_alias).create(name='ssh', port=22, platform=platform)
...
def _create_account_obj(secret, secret_type, gateway, asset, account_model):
    return account_model(
        ...
        name=f'{gateway.name}-{secret_type}-gateway',
    )
...
def migrate_gateway_to_asset(apps, schema_editor):
    ...
    platform_model = apps.get_model('assets', 'Platform')
    gateway_platform = platform_model.objects.using(db_alias).get(name='Gateway')
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(create_internal_gateway_platform),
