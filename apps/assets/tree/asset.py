...
class AssetTree(object):
    ...
    * (节省内存空间 => 所有资产id在内存中只保留一份)
    org_mapping_to_assets_id_with_themselves_mapping = {
        org_id: {
            asset_id: asset_id,
            asset_id: asset_id,
        }
    ...
    * (提高计算树(资产树/用户授权树)节点下资产数量的速度)
    org_mapping_to_node_key_with_all_assets_id_mapping = {
        org_id: {
            node_key: [asset_id, asset_id]
        }
    }

    """

    # { org_id: { asset_id: asset_id } } (节省内存空间)
    org_mapping_to_assets_id_with_themselves_mapping = defaultdict(dict)
    org_mapping_to_node_key_with_all_assets_id_mapping = defaultdict(dict)
    ...
    def get_all_assets_id_of_node(self, node_key):
        """ 获取节点下的所有资产id """
        _mapping = self.org_mapping_to_node_key_with_all_assets_id_mapping[self._org_id]
    ...
    def initial(self):
        ...
        self._initial_tree()
        # 7s
        self._construct_node_key_with_all_assets_id_mapping()
    ...
    def _initial_tree(self):
        ...
        assets_id_mapping = self._construct_assets_id_with_themselves_mapping()
    ...
    def _construct_assets_id_with_themselves_mapping(self):
        """
        构造资产id与自身的映射关系
        * (节省内存空间)
        """
        _mapping = self.org_mapping_to_assets_id_with_themselves_mapping[self._org_id]
        if not _mapping:
            _mapping = self._get_assets_id_with_themselves_mapping()
            self.org_mapping_to_assets_id_with_themselves_mapping[self._org_id] = _mapping
        return _mapping

    def _construct_node_key_with_all_assets_id_mapping(self):
        """
        构造节点key与所有资产id的映射关系
        * (提高计算树节点下资产数量的速度)
        """
        ...
        if not _mapping:
            ...
            self.org_mapping_to_node_key_with_all_assets_id_mapping[self._org_id] = _mapping
        t2 = time.time()
        print('time => _construct_node_key_with_all_assets_id_mapping: {}'.format(t2-t1))
        return _mapping
