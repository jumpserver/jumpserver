    ...
    def initial(self):
        ...
        with tmp_to_org(self._org_id):
            ...
            nodes_assets_id = list(
                Asset.nodes.through.objects.filter(node__in=nodes_id).values_list('node_id', 'asset_id')
            )
            t2 = time.time()
        ...
        for node in nodes:
            ...
            for asset_id in node_id_assets_id_mapping[str(node.id)]:
                asset_tree_node = self.append_path_to_tree_node(
                    tree_node=asset_tree_node_parent, arg_path=asset_id
                )
