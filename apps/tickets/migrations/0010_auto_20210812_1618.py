...
def update_ticket_process(apps, schema_editor):
    ...
    with transaction.atomic():
        ...
        ticket_model.objects.bulk_update(updates, ['process_map', ])
    ...
    with transaction.atomic():
        for id, assignees, action, created_by in ticket_assignee_m2m:
            if action == 'open':
                ...
                state = 'rejected'
            step_instance = ticket_step_model.objects.create(ticket_id=id, state=state, created_by=created_by)
...
def create_ticket_flow_and_approval_rule(apps, schema_editor):
    ...
    super = user_model.objects.filter(role='Admin')
    with transaction.atomic():
        apply_asset = ticket_flow_model.objects.create(
            title='global_apply_asset', type='apply_asset', org_id=org_id, approval_level=2)
        apply_application = ticket_flow_model.objects.create(
            title='global_apply_application', type='apply_application', org_id=org_id, approval_level=2)
        creates = list()
        for obj in (apply_asset, apply_application):
            for i in [1, 2]:
                instance = approval_rule_model.objects.create(level=i, strategy='super',
                                                              assignees_display=['{0.name}({0.username})'.format(i) for i in super])
                instance.assignees.set(list(super))
                creates.append(instance)
            obj.rules.set(creates)
            creates = list()
