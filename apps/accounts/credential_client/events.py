...
def enqueue(event, code, clients=None):
    ...
    if event.event != AuditEvent.SUBSCRIPTION_SNAPSHOT:
        enqueue_webhooks(event, code)


def enqueue_webhooks(event, code):
    from accounts.models import ApplicationWebhook
    ...
    for webhook in webhooks:
        ...
        try:
            ...
            # Invalid persisted configuration must not roll back the business event.
            logger.warning('Skipping invalid application webhook %s.', webhook.id)
            continue
        try:
            with transaction.atomic():
                if ApplicationEventDelivery.objects.filter(event=event, webhook=webhook).exists():
                    continue
                audit = record(
